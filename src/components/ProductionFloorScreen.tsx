...
const productionFloorInterior = require('../../assets/recon-opus-production-floor-interior.png');

type Props = { departments: Department[]; revisionCount: number; subletCount: number; motionEnabled?: boolean; movement?: FacilityMovement | null; onMovementComplete?: (movementId: number) => void; onOpenDepartment: (name: ProductionDepartmentName) => void; onBackToLobby: () => void; onOpenRevisions: () => void; onOpenSublets: () => void; onOpenHistory: () => void };

export function ProductionFloorScreen({ departments, revisionCount, subletCount, motionEnabled = true, movement, onMovementComplete, onOpenDepartment, onBackToLobby, onOpenRevisions, onOpenSublets, onOpenHistory }: Props) {
  ...
  return <View style={styles.page}>
    <View pointerEvents="none" style={StyleSheet.absoluteFill}><Image source={productionFloorInterior} resizeMode="cover" style={styles.environmentBackground} /></View>
    ...
    <View style={styles.header}>
      ...
      <Pressable onPress={onBackToLobby} accessibilityRole="button" style={({ pressed }) => [styles.lobbyBack, pressed && styles.lobbyBackPressed]}><Text style={styles.lobbyBackIcon}>‹</Text><Text style={styles.lobbyBackText}>Lobby</Text></Pressable>
    ...
    <View style={styles.map} onLayout={onMapLayout}>
      ...
      <View
        pointerEvents="none"
        accessibilityLabel="Entrance"
        style={[styles.entranceGateway, { borderColor: workflowColors.Entrance }]}
      >
        <Text style={[styles.entranceMark, { color: workflowColors.Entrance }]}>↘</Text>
        <View>
          <Text numberOfLines={1} style={[styles.gatewayLabel, { color: workflowColors.Entrance }]}>ENTRANCE</Text>
          <Text numberOfLines={1} style={styles.gatewayHint}>PRODUCTION IN</Text>
        </View>
      </View>
      {departments.filter(({ name }) => name !== 'Revision Needed' && name !== 'Entrance').map((department) => {
        ...
        return <Pressable key={name} onPress={() => onOpenDepartment(name)} accessibilityRole="button" accessibilityLabel={`Enter ${name}, ${department.vehicles.length} vehicles`} style={({ pressed }) => [styles.zone, { left: `${point.x * 100 - 18}%`, top: `${point.y * 100 - 8.5}%`, borderColor: highlighted ? color : `${color}55`, shadowColor: color, shadowOpacity: highlighted ? 0.65 : 0 }, highlighted && styles.zoneHighlighted, pressed && styles.pressed]}>
    ...
    <View style={[styles.historySection, { paddingBottom: insets.bottom + 8 }]}>
...
const styles = StyleSheet.create({
  page: { flex: 1, backgroundColor: colors.background }, environmentBackground: { ...StyleSheet.absoluteFillObject, width: '100%', height: '100%' }, environmentWash: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(3, 7, 10, 0.55)' }, header: { height: 50, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: colors.border, overflow: 'hidden' }, headerImageUnderlay: { ...StyleSheet.absoluteFillObject }, headerImage: { width: '100%', height: '100%', opacity: 0.16 }, headerImageShade: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(7, 10, 13, 0.64)' }, headerImageFade: { position: 'absolute', left: 0, right: 0, bottom: 0, height: 16, backgroundColor: 'rgba(7, 10, 13, 0.42)' }, lobbyBack: { position: 'absolute', left: 8, top: 4, minHeight: 42, paddingHorizontal: 8, flexDirection: 'row', alignItems: 'center', zIndex: 2 }, lobbyBackPressed: { opacity: 0.65 }, lobbyBackIcon: { color: colors.accent, fontSize: 25, lineHeight: 27, marginRight: 3 }, lobbyBackText: { color: colors.text, fontSize: 10, fontWeight: '800' }, title: { color: colors.text, fontSize: 24, fontWeight: '900', letterSpacing: -0.8 }, content: { flex: 1 }, contentContainer: { flexGrow: 1 },
  ...
  map: { flex: 1, margin: 7, marginTop: 5, overflow: 'visible', borderWidth: 1, borderColor: 'rgba(89, 112, 130, 0.38)', borderRadius: 14, backgroundColor: 'rgba(7, 12, 16, 0.76)' }, floorOutline: { position: 'absolute', left: '3%', right: '3%', top: '1%', bottom: '1%', borderWidth: 1, borderColor: 'rgba(83, 105, 121, 0.28)', borderRadius: 22 },
  entranceGateway: { position: 'absolute', left: '2.2%', top: '1.2%', minHeight: 31, width: '27%', paddingHorizontal: 8, flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(15, 22, 28, 0.92)', borderWidth: 1, borderRadius: 8, zIndex: 5 }, entranceMark: { fontSize: 15, fontWeight: '900', marginRight: 6 }, gatewayLabel: { color: '#DCE8F0', fontSize: 9, lineHeight: 11, fontWeight: '900', letterSpacing: 0.8 }, gatewayHint: { color: '#677887', fontSize: 5.5, lineHeight: 8, fontWeight: '900', letterSpacing: 0.65 },
  zone: { position: 'absolute', width: '36%', height: '17%', minHeight: 72, backgroundColor: 'rgba(12, 18, 23, 0.91)', borderWidth: 1, borderRadius: 8, paddingVertical: 8, paddingHorizontal: 7, overflow: 'hidden', zIndex: 2, shadowRadius: 10 }, zoneHighlighted: { borderWidth: 2 }, zoneStripe: { position: 'absolute', left: 0, top: 0, bottom: 0, width: 4 }, zoneTop: { minHeight: 28, flexDirection: 'row', alignItems: 'flex-start', gap: 5 }, zoneName: { flex: 1, color: colors.text, fontSize: 12.5, lineHeight: 14, fontWeight: '900' }, count: { minWidth: 27, height: 27, borderRadius: 14, alignItems: 'center', justifyContent: 'center' }, countText: { fontSize: 12, fontWeight: '900' }, oldest: { color: colors.subtle, fontSize: 6.5, fontWeight: '900', letterSpacing: 0.6, marginTop: 2 }, preview: { gap: 2, marginTop: 3 }, previewRow: { flexDirection: 'row', alignItems: 'center' }, dot: { width: 4, height: 4, borderRadius: 2, marginRight: 5 }, previewText: { flex: 1, color: '#AEB5BC', fontSize: 7.5, fontWeight: '700' }, openBay: { color: colors.subtle, fontSize: 7, fontWeight: '900', letterSpacing: 1 }, alerts: { position: 'absolute', right: 7, bottom: 4, flexDirection: 'row', gap: 4 }, priority: { color: colors.accent, fontSize: 5.5, fontWeight: '900' }, aging: { color: '#FB8B65', fontSize: 5.5, fontWeight: '900' }, pressed: { opacity: 0.72, transform: [{ scale: 0.98 }] },
  historySection: { minHeight: 60, paddingTop: 8, justifyContent: 'center' }, historyRow: { minHeight: 44, marginHorizontal: 8, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: colors.border, borderRadius: 9, backgroundColor: 'rgba(13, 19, 24, 0.91)' }, historyRowIcon: { color: colors.muted, fontSize: 17, fontWeight: '700', marginRight: 9 }, historyRowText: { flex: 1, color: colors.muted, fontSize: 12, fontWeight: '800' }, historyRowChevron: { color: colors.subtle, fontSize: 21, fontWeight: '600' },
